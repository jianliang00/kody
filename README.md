...
| `CODY_REQUIRE_SHELL_APPROVAL` | `true` | 是否要求客户端批准模型发起的 Shell 调用 |

## 安全边界

- 文件工具只能访问 Workspace 或当前 Thread 已引用的 Project，并拒绝绝对路径、`..` 和符号链接逃逸。
- Shell 使用净化后的环境，不继承 Provider Key、Server Token 或用户 Cargo 凭据；但它不是操作系统级文件沙箱。因此默认必须审批 Shell，部署方仍应在容器、Seatbelt、Landlock 等外部沙箱中运行服务。
