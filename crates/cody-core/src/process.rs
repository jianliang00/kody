...
const LOG_MAGIC: &[u8; 8] = b"CODYPLG1";
...
impl ProcessManager {
    ...
    pub async fn start(&self, request: StartProcessRequest) -> Result<ManagedProcess> {
        ...
        started_rx
            .await
            .map_err(|_| CodyError::Tool("managed process actor stopped during startup".into()))?
            .map_err(CodyError::Tool)
    }
    ...
    pub async fn read_output(
        ...
        if process.output_end_cursor > 0 && log.end_cursor == 0 {
            return Err(CodyError::Store(format!(
    ...
    pub async fn stop(&self, thread_id: ThreadId, process_id: ProcessId) -> Result<ManagedProcess> {
        ...
        loop {
            if let Some(outcome) = completion.borrow().clone() {
                return outcome.map_err(CodyError::Tool);
            }
...
fn process_manager_shutting_down() -> CodyError {
    CodyError::Conflict("process manager is shutting down and cannot start new work".into())
}
...
impl OutputLog {
    ...
    async fn load(path: &Path, max_bytes: usize, max_records: usize) -> Result<Self> {
        ...
        Self::decode(&bytes, max_bytes, max_records).map_err(CodyError::Io)
    }
...
mod tests {
    ...
    async fn shutdown_barrier_rejects_every_later_start() {
        ...
        assert!(matches!(
            error,
            CodyError::Conflict(message) if message.contains("shutting down")
        ));
