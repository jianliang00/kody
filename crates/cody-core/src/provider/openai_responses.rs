...
impl OpenAiResponsesProvider {
    pub fn new(config: OpenAiResponsesConfig) -> Result<Self> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|error| {
                CodyError::Provider(format!("failed to build Responses client: {error}"))
            })?;
        Ok(Self {
            config,
            client,
    ...
    fn transport_error(&self, operation: &str, error: reqwest::Error) -> CodyError {
    ...
    fn redact_error(&self, error: CodyError) -> CodyError {
        match error {
            CodyError::Provider(message) => CodyError::Provider(self.redact(&message)),
            other => other,
        }
    }
}
...
fn endpoints(base_url: &str) -> Result<(Url, Url)> {
    ...
    let parsed = Url::parse(root).map_err(|error| {
        CodyError::InvalidInput(format!("invalid Responses base URL '{base_url}': {error}"))
    })?;
    ...
    let responses = Url::parse(&format!("{root}/responses"))
        .map_err(|error| CodyError::InvalidInput(error.to_string()))?;
...
fn encode_request(request: &ModelRequest, model: String) -> Result<Value> {
    ...
    if let Some(temperature) = request.temperature {
        let value = serde_json::Number::from_f64(temperature as f64)
            .ok_or_else(|| CodyError::InvalidInput("temperature must be a finite number".into()))?;
...
fn protocol_error(message: impl Into<String>) -> CodyError {
    provider_failure(ProviderFailure::new(ProviderErrorKind::Protocol, message))
}

fn provider_failure(failure: ProviderFailure) -> CodyError {
    CodyError::Provider(failure.to_string())
}
...
mod tests {
    ...
    async fn cancellation_drops_a_slow_stream() {
        ...
        cancellation.cancel();
        assert!(matches!(task.await.unwrap(), Err(CodyError::Cancelled)));
        server.abort();
    }
