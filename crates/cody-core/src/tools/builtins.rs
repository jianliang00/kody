...
impl Tool for ReadFileTool {
    ...
    async fn execute(&self, call: &ToolCall, context: &ToolContext) -> Result<ToolResult> {
        ...
        tokio::select! {
            biased;
            _ = context.cancellation_token.cancelled() => return Err(CodyError::Cancelled),
...
impl Tool for WriteFileTool {
    ...
    async fn execute(&self, call: &ToolCall, context: &ToolContext) -> Result<ToolResult> {
        ...
        let parent = target.parent().ok_or_else(|| {
            CodyError::InvalidInput(format!("invalid file path '{}'", arguments.path))
        })?;
...
impl Tool for ShellTool {
    ...
    async fn execute(&self, call: &ToolCall, context: &ToolContext) -> Result<ToolResult> {
        ...
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| CodyError::Tool("failed to capture shell standard output".to_owned()))?;
        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| CodyError::Tool("failed to capture shell standard error".to_owned()))?;
    ...
    let root = match project_id {
        ...
        Some(project_id) => {
            ...
            if requires_write && binding.access == ProjectAccess::ReadOnly {
                return Err(CodyError::Tool(format!(
...
async fn confined_write_path(root: &Path, relative: &Path) -> Result<PathBuf> {
    ...
    Err(CodyError::InvalidInput(format!(
    ...
    T: for<'de> Deserialize<'de>,
{
    serde_json::from_value(call.arguments.clone()).map_err(|error| {
        CodyError::InvalidInput(format!("invalid arguments for '{}': {error}", call.name))
    })
}
...
async fn cancellable<T>(
    ...
    tokio::select! {
        biased;
        _ = cancellation_token.cancelled() => Err(CodyError::Cancelled),
...
async fn collect_reader(mut task: JoinHandle<io::Result<BoundedOutput>>) -> Result<BoundedOutput> {
    match timeout(Duration::from_secs(1), &mut task).await {
        Ok(joined) => joined
            .map_err(|error| CodyError::Tool(format!("output reader task failed: {error}")))?
            .map_err(CodyError::Io),
        Err(_) => {
            task.abort();
            let _ = task.await;
            Err(CodyError::Tool(
...
pub(super) mod tests {
    ...
    async fn read_only_project_allows_reads_but_blocks_write_and_shell() {
        ...
        for (name, arguments) in [
            ...
            assert!(matches!(
                registry.execute(&call, &context).await,
                Err(CodyError::Tool(message)) if message.contains("read-only")
            ));
        }
    }
    ...
    async fn shell_honors_cancellation() {
        ...
        assert!(matches!(error, CodyError::Cancelled));
