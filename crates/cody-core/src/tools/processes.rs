...
fn validate_environment_with_limits(
    ...
    for (name, value) in environment {
        ...
        bytes = bytes
            ...
            .ok_or_else(|| CodyError::InvalidInput("environment is too large".to_owned()))?;
    }
    if bytes > max_bytes {
        return Err(CodyError::InvalidInput(format!(
...
mod tests {
    ...
    async fn start_process_rejects_a_symlink_cwd_outside_the_selected_root() {
        ...
        let error = tool
            ...
            .await
            .unwrap_err();
        assert!(matches!(error, CodyError::InvalidInput(message) if message.contains("outside")));
    }
    ...
    async fn start_process_rejects_a_read_only_project() {
        ...
        let error = tool
            ...
            .await
            .unwrap_err();
        assert!(matches!(error, CodyError::Tool(message) if message.contains("read-only")));
    }
    ...
    async fn engine_process_tools_cover_start_list_output_and_stop_across_turn_cancellation() {
        ...
        let engine = CodyEngine::in_memory(EngineConfig {
