...
impl AppState {
    pub fn new(engine: Arc<KodyEngine>) -> Self {
        let auth_token = std::env::var("KODY_SERVER_TOKEN")
            .ok()
        ...
        std::env::remove_var("KODY_SERVER_TOKEN");
        let allowed_origins = std::env::var("KODY_ALLOWED_ORIGINS")
            .ok()
            .into_iter()
            .flat_map(|origins| {
                origins
...
impl TurnManager {
    pub fn register_backend(
        ...
        if provider_id.trim().is_empty() {
            return Err(kody_core::KodyError::InvalidInput(
        ...
        self.backends
            .write()
            .map_err(|_| {
                kody_core::KodyError::Store("turn backend registry lock was poisoned".into())
            })?
            .insert(provider_id, backend);
        Ok(())
    }

    pub async fn start(&self, runtime: Arc<AgentRuntime>, request: StartTurn) -> KodyResult<Turn> {
    ...
    pub async fn prepare(
        ...
        request: StartTurn,
    ) -> KodyResult<(Turn, CancellationToken)> {
