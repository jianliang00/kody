...
impl OpenAiCompatibleConfig {
    ...
    ///
    /// `KODY_OPENAI_*` variables take precedence over their conventional
    ...
    ///
    /// Supported variables:
    /// - `KODY_OPENAI_PROVIDER_ID`
    /// - `KODY_OPENAI_BASE_URL` / `OPENAI_BASE_URL`
    /// - `KODY_OPENAI_API_KEY` / `OPENAI_API_KEY`
    /// - `KODY_OPENAI_MODEL` / `OPENAI_MODEL`
    /// - `KODY_OPENAI_ORGANIZATION` / `OPENAI_ORGANIZATION`
    /// - `KODY_OPENAI_PROJECT` / `OPENAI_PROJECT`
    /// - `KODY_OPENAI_TIMEOUT_SECS`
    pub fn from_env() -> Result<Option<Self>> {
        let id = optional_env("KODY_OPENAI_PROVIDER_ID")?;
        let kody_base_url = optional_env("KODY_OPENAI_BASE_URL")?;
        let standard_base_url = optional_env("OPENAI_BASE_URL")?;
        let kody_api_key = optional_env("KODY_OPENAI_API_KEY")?;
        let standard_api_key = optional_env("OPENAI_API_KEY")?;
        let kody_model = optional_env("KODY_OPENAI_MODEL")?;
        let standard_model = optional_env("OPENAI_MODEL")?;
        let kody_organization = optional_env("KODY_OPENAI_ORGANIZATION")?;
        let standard_organization = optional_env("OPENAI_ORGANIZATION")?;
        let kody_project = optional_env("KODY_OPENAI_PROJECT")?;
        let standard_project = optional_env("OPENAI_PROJECT")?;
        let timeout = optional_env("KODY_OPENAI_TIMEOUT_SECS")?;

        let configured = id.is_some()
            || kody_base_url.is_some()
            || standard_base_url.is_some()
            || kody_api_key.is_some()
            || standard_api_key.is_some()
            || kody_model.is_some()
            || standard_model.is_some()
            || kody_organization.is_some()
            || standard_organization.is_some()
            || kody_project.is_some()
            ...
            .map(|value| {
                value.parse::<u64>().map_err(|_| {
                    KodyError::InvalidInput(
                        "KODY_OPENAI_TIMEOUT_SECS must be a positive integer".into(),
                    )
                })
            })
            .transpose()?
        ...
        if timeout == 0 {
            return Err(KodyError::InvalidInput(
                "KODY_OPENAI_TIMEOUT_SECS must be greater than zero".into(),
            ));
        }
...
impl OpenAiCompatibleProvider {
    pub fn new(config: OpenAiCompatibleConfig) -> Result<Self> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|error| {
                KodyError::Provider(format!("failed to build OpenAI HTTP client: {error}"))
            })?;
        Ok(Self {
            config,
            endpoint,
    ...
    fn provider_error(&self, message: impl Into<String>) -> KodyError {
        KodyError::Provider(self.redact(&message.into()))
    }

    fn redact_error(&self, error: KodyError) -> KodyError {
        match error {
            KodyError::Provider(message) => self.provider_error(message),
            other => other,
        }
    }
}
...
fn optional_env(key: &str) -> Result<Option<String>> {
    match env::var(key) {
        ...
        Err(env::VarError::NotUnicode(_)) => Err(KodyError::InvalidInput(format!(
...
fn decode_response(response: OpenAiResponse) -> Result<ModelResponse> {
    let choice = response.choices.into_iter().next().ok_or_else(|| {
        KodyError::Provider("OpenAI-compatible response contained no choices".into())
    })?;
...
fn decode_text_content(content: Value) -> Result<Option<String>> {
    match content {
        ...
        other => Err(KodyError::Provider(format!(
...
mod tests {
    ...
    fn assert_secret_redacted(error: &KodyError, secret: &str) {
