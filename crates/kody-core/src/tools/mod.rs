...
impl ToolRegistry {
    ...
    /// Construct a registry containing Kody's filesystem and shell tools.
    ...
    pub async fn execute(&self, call: &ToolCall, context: &ToolContext) -> Result<ToolResult> {
        ...
        let tool = self
            .get(&call.name)
            .ok_or_else(|| KodyError::ToolNotFound(call.name.clone()))?;
...
mod tests {
    ...
    async fn unknown_tool_is_reported_by_registry() {
        ...
        assert!(matches!(error, KodyError::ToolNotFound(name) if name == "missing"));
    }
}
