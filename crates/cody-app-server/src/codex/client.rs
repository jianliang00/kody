...
impl Default for CodexClientOptions {
    fn default() -> Self {
        Self {
            ...
            client_name: "cody".into(),
            client_title: "Cody".into(),
...
impl CodexClient {
    ...
    /// explicit `CODY_CODEX_PATH` remains authoritative. Each attempted binary
    ...
    pub async fn discover_and_spawn(options: CodexClientOptions) -> Result<Self> {
        ...
        if candidates.is_empty() {
            return Err(CodexError::NoUsableBinary {
                attempts: "CODY_CODEX_PATH was unset and no executable was found in PATH or configured bundles".into(),
            });
        }
    ...
    /// Starts Codex-managed device-code OAuth. Cody never receives OAuth
...
fn handle_server_request(
    ...
    // delegated through Cody's generic server-request stream.
    if method.to_ascii_lowercase().contains("chatgptauthtokens") {
        send_automatic_rejection(
            Arc::downgrade(inner),
            id,
            -32601,
            "external token auth is not supported by Cody",
        );
        return Ok(());
    }
    {
    ...
    if inner.server_requests.send(request).is_err() {
        ...
        send_automatic_rejection(
            ...
            id,
            -32601,
            "no Cody server-request handler is subscribed",
        );
        return Ok(());
    }
    ...
    tokio::spawn(async move {
        ...
        if expired {
            send_automatic_rejection(
                ...
                id,
                -32001,
                "Cody server-request handler timed out",
            );
        }
    });
    Ok(())
