...
use cody_app_server::AppState;
use cody_core::{AgentEvent, CodyEngine, EngineConfig, StartTurn, DEFAULT_THREAD_TITLE};

/// Manual end-to-end proof that Cody can execute a durable Turn through the
...
async fn executes_a_real_turn_with_codex_plan_quota() {
    ...
    let turn = state
        ...
        .start(
            ...
            StartTurn {
                thread_id: thread.id,
                message: "Reply with exactly CODY_CODEX_SMOKE_OK. Do not call tools.".into(),
                references: Vec::new(),
                provider: "codex".into(),
                model: Some(model),
                temperature: None,
    ...
    assert!(final_text.contains("CODY_CODEX_SMOKE_OK"));

    let messages = engine.store().list_messages(thread.id).await.unwrap();
    assert!(messages
        .iter()
        .any(|message| message.role == cody_core::MessageRole::Assistant
            && message.text().contains("CODY_CODEX_SMOKE_OK")));
