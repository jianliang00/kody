...
4. Inspector (approximately 20rem): one independently scrollable column of disclosure sections for Current Thread context, Workspace, active references, background processes, changed files, execution timeline, and Projects.
...
Use native macOS desktop hierarchy: a darker translucent source-list Workbench, white Thread list and inspector surfaces, a quiet gray toolbar, compact controls, thin split separators, and system typography. Warm orange is Kody's brand color for New Thread, the selected Thread, the active user-message rule, and Send; system `AccentColor` remains reserved for focus rings and standard control semantics. Permanent regions stay flat. Rounded containers and shadows are limited to the selected Thread, temporary controls, menus, dialogs, and the composer.

Typography is intentionally scoped by role. Conversation copy and ordinary controls use `body` (14px). Dense source-list titles use 13px, Thread metadata uses 12px, and other secondary metadata uses `caption` (13px). The toolbar title is compact; the active Thread receives its real heading and status hierarchy in the document header below it. Sidebar section and column headings use medium 500, Thread row titles use semibold 600, and navigation labels and metadata remain regular 400; other controls and local emphasis use medium 500, while headings and status badges use semibold 600 at most. The composer starts at two text rows, keeps provider/model and context tools in the bottom action area, and may resize vertically up to its bounded maximum; its measured dock height remains authoritative for timeline and floating-shelf clearance.
