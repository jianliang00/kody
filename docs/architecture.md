...
The sidecar is the authentication authority. Kody uses its account login/logout and rate-limit methods, model catalog, Thread/Turn methods, notifications, server requests, and interruption method. It never reads `~/.codex/auth.json`, receives an OAuth/refresh token, or converts a ChatGPT token into an API key. The trusted host may set `KODY_CODEX_PATH`; Renderer content cannot select a binary. The catalog advertises supported reasoning efforts and Speedy availability, and the validated Turn contract persists those choices before the Codex adapter maps them to `effort` and the `priority` service tier.

For a Codex Turn, Kody resolves its normal Workspace and reference context, starts or resumes the opaque external Thread binding, selects the requested model, and sets the Workspace as `cwd`. The durable Turn permission mode is translated at thread start/resume and turn start: `read_only` uses `never` plus a read-only/no-network sandbox, `ask` uses `on-request` plus workspace-write roots for the Thread Workspace and read-write Project bindings, and `full_access` uses `never` plus danger-full-access. Kody remains the explicit user reviewer. Codex notifications become Kody model/reasoning/tool/file events. A Kody cancellation interrupts the remote Turn, and dropping an in-flight backend future also arms a best-effort interrupt guard.

The native agent loop applies the same provider-neutral modes before tool execution. Read-only tools are the only definitions exposed in `read_only`, and an undeclared write/command call is still rejected at execution time. `ask` preserves normal file tools but routes command execution through `ApprovalBroker`; `full_access` skips that interactive approval. Native shell execution is not an OS sandbox boundary, so full access is intentionally presented as a high-risk mode.
